...
function headingTabs(files:{name:string,content:string}[]){
  return (
    <div className="bg-[#191919] text-white text-sm py-2 h-fit">
      {files.map((file)=><span key={file.name} className="px-4 cursor-pointer hover:bg-white/10 py-4">{file.name}</span>)}
    </div>
  )
}
